...
class AudioDataset(data.Dataset):
    ...
    def __init__(self, data_type, dataset_name):

        pre = '../datasets/' + dataset_name + '/'

        if data_type == 'train':
            data_path = pre + serialized_train_folder
        else:
            data_path = pre + serialized_test_folder
...
def PESQ(ref_wav, deg_wav):
    ...
    pesq_score = pesq(16000, ref_wav, deg_wav, 'wb')
    return pesq_score
    # # reference wav
    # # degraded wav
    ...
    # try:
    #     p = pesq(ref_tfl, deg_tfl, 16000, 'wb')
...
def CompositeEval(ref_wav, deg_wav, log_all=False):
    ...
    # Compute the SSNR
    # print("ref_wav {}, deg_wav {}".format(ref_wav, deg_wav))
